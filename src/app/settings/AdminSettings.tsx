...
type ApiType = { id: string, name: string, url: string, isActive: boolean, failCount: number };
    ...
    <div className="space-y-8">
      <Card className="shadow-sm border-border bg-card relative overflow-hidden">
        ...
        <CardHeader className="border-b border-border pb-4 pt-6">
          ...
          <CardDescription>Adjust the global billing parameters and support channels.</CardDescription>
        </CardHeader>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 items-end">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-background focus-visible:ring-[#00D2FF] font-mono text-foreground"
              />
            </div>
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-muted-foreground"><Server className="w-3.5 h-3.5" /> Max CSV Import Rows</Label>
              <Input
                type="number"
                value={csvLimit}
                onChange={(e) => setCsvLimit(e.target.value)}
                className="bg-background focus-visible:ring-[#00D2FF] font-mono text-foreground"
              />
            </div>
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-muted-foreground"><MessageCircle className="w-3.5 h-3.5" /> Support Telegram Handle</Label>
              <Input
                value={tgHandle}
                onChange={(e) => setTgHandle(e.target.value)}
                className="bg-background focus-visible:ring-[#00D2FF] text-foreground"
              />
            </div>
      ...
      <Card className="shadow-sm border-border bg-card h-full">
        <CardHeader className="border-b border-border pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription className="mt-1">Manage active SMS API nodes. Traffic is distributed evenly across ONLINE nodes.</CardDescription>
          </div>
          <AddApiModal />
        </CardHeader>
